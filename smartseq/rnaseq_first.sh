...
# 运行开关：可在命令前以环境变量覆盖，例如 FORCE=1 bash rnaseq_first.sh --all。
THREADS="${THREADS:-8}"                    # 并行线程数。
FORCE="${FORCE:-1}"                        # 1：忽略已有输出并重跑；链设定修正后重跑必须设为 1。
...
run_mapping_stage() {
    ...
    for idx in "${!SAMPLE_NAMES[@]}"; do
        ...
        if [[ "${DRY_RUN}" == "1" ]]; then
            log "+ ${HISAT2_BIN} --new-summary -p ${THREADS} -x ${HISAT2_INDEX} -1 ${clean_r1} -2 ${clean_r2} | ${SAMTOOLS_BIN} sort -@ ${THREADS} -o ${bam} -"
        ...
        # PN5 文库已确认非链特异性：不要传入 --rna-strandness。
        "${HISAT2_BIN}" \
            --new-summary \
            -p "${THREADS}" \
            -x "${HISAT2_INDEX}" \
...
write_run_provenance() {
...

    {
        ...
        printf 'HISAT2\t(no --rna-strandness)\tnon-strand-specific library\n'
        printf 'featureCounts\tstrandSpecific=0\tnon-strand-specific counting\n'
        printf 'rMATS\tfr-unstranded\tkept consistent with featureCounts in rnaseq_third.sh\n'
        printf 'APA\tNONE\tAPA strand mode derived from fr-unstranded\n'
        printf 'validation\tRSeQC infer_experiment.py\tPN5 library empirically confirmed as non-strand-specific\n'

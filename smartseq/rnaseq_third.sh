...
# 运行开关：可在命令前以环境变量覆盖，例如 FORCE=1 RNASEQ_THIRD_METHODS=rmats bash rnaseq_third.sh。
...
APA_STRANDTYPE="${APA_STRANDTYPE:-auto}"    # auto：由 RMATS_LIBTYPE 推导；非链特异性时为 NONE。
